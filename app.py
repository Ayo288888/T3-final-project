...
def calculate_performance(df):
    """
    Takes a raw DataFrame (from CSV or Manual Input), cleans it,
    and calculates GPA/CGPA stats.
    """
    try:
        ...
        if not required.issubset(df.columns):
            return None, None, f"Missing columns. Required: {required}"
        ...
        df = df.dropna(subset=['Unit', 'Semester']) # Drop rows if unit/sem is invalid

        df['Point'] = df['Grade'].apply(get_point)
        df['Weighted_Point'] = df['Unit'] * df['Point']

        if df.empty:
            return None, None, "No valid data found (check units/grades)."
        ...
        semester_stats = df.groupby('Semester').apply(
            lambda x: pd.Series({
                ...
                'GPA': x['Weighted_Point'].sum() / x['Unit'].sum() if x['Unit'].sum() > 0 else 0
            })
        ).reset_index()
...
with st.sidebar:
    ...
    # --- DATA SOURCE TOGGLE ---
    data_source = st.radio("Data Source", ["📂 Upload CSV", "✍️ Manual Entry"])
...
if data_source == "📂 Upload CSV" and uploaded_file:
    ...
    df_final, stats_final, error_msg = calculate_performance(raw_df)

# Case 2: Manual Entry
elif data_source == "✍️ Manual Entry":
    st.info("💡 Enter your grades below. You can add more rows by clicking the '+' at the bottom of the table.")
    
    # Template for manual entry
    default_data = pd.DataFrame(
        [
            {"Semester": 1, "Course": "MTH101", "Unit": 3, "Grade": "A"},
            {"Semester": 1, "Course": "PHY101", "Unit": 2, "Grade": "B"},
        ]
    )
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            ...
            "Semester": st.column_config.NumberColumn(
                "Semester", min_value=1, max_value=12, step=1, format="%d"
            ),
             "Unit": st.column_config.NumberColumn(
                "Unit", min_value=0, max_value=20, step=1
            ),
        },
    ...
    if st.button("🚀 Generate Dashboard"):
        df_final, stats_final, error_msg = calculate_performance(edited_df)

# --- DASHBOARD LOGIC ---
if error_msg:
    st.error(f"❌ Error: {error_msg}")

elif df_final is not None and stats_final is not None:
    # --- Top Metrics Row ---
    col1, col2, col3, col4 = st.columns(4)
    
    curr_cgpa = stats_final['CGPA'].iloc[-1]
    last_gpa = stats_final['GPA'].iloc[-1]
    
    if len(stats_final) > 1:
        prev_gpa = stats_final['GPA'].iloc[-2]
    ...
    col3.metric("Total Units", f"{int(stats_final['Cumulative_Units'].iloc[-1])}")
    col4.metric("Semesters Done", f"{stats_final['Semester'].max()}")
    ...
    with tab1:
        ...
        st.pyplot(plot_dashboard(df_final, stats_final))
    ...
    with tab2:
        ...
        with c1:
            ...
            pred_gpa, err = predict_gpa_ml(stats_final)
